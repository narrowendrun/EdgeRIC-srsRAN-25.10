                                                             ...
                                                             unsigned                max_ue_grants_to_alloc)
{
  ...
  for (const auto& ue_candidate : newtx_candidates) {
    ...
    if (result.has_value()) {
      ...
      // EdgeRIC: When using weight-based allocation, account for actual weighted PRBs
      // instead of expected_nof_rbs to allow multiple UEs per slot
      // Uses staleness check: weights expire if muApp stops sending
      unsigned estimated_prbs = grant_builder.context().expected_nof_rbs;
      uint16_t rnti = static_cast<uint16_t>(grant_builder.context().rnti);
      std::optional<float> opt_weight = edgeric::get_weights(rnti);
      if (opt_weight.has_value() && opt_weight.value() > 0.0f) {
        // Estimate PRBs based on weight: weight * total_prbs
        estimated_prbs = static_cast<unsigned>(opt_weight.value() * static_cast<float>(rbs_to_alloc));
        estimated_prbs = std::max(estimated_prbs, 1U);  // At least 1 PRB
        estimated_prbs = std::min(estimated_prbs, grant_builder.context().expected_nof_rbs);
      }
      
      rb_count += std::min(estimated_prbs, max_rbs_per_grant);
